...
**Independent Test**: Start the app, go to "New Character", enter name, confirm, see that name on the Menu screen, and check that a YAML file is generated in the user data directory containing this name.
...
**Independent Test**: Start the game with an active character, play until target distance is reached, verify transition to the Game Won screen, and check that the inventory is updated.
...
**Independent Test**: Press 'c' on the Menu, verify transition to the Create Potions placeholder screen, and press back to return to the Menu.
...
- **FR-002**: System MUST persist character profiles (name, levels_completed, inventory) in a YAML configuration file (`config.yaml`) within the OS-appropriate user data directory.
